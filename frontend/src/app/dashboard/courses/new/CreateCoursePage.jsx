...
export default function CreateCoursePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        err.response?.data?.message === "Course code already exists" ||
        err.response?.status === 409
      ) {
        setCourseCodeError(
          "Course code already exists. Please choose another one.",
        );
      } else {
        handleError(err);
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <input
                ...
                required
                className={`form-input border rounded-md p-2 w-full mt-2 ${
                  courseCodeError ? "border-red-500 bg-red-50" : ""
                }`}
              ...
              />
              {courseCodeError && (
                <p className="text-red-500 text-sm mt-1">{courseCodeError}</p>
              )}
            </div>
          </div>
